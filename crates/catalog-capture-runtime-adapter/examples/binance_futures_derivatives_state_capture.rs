...
use catalog_capture_core::{
    ...
    plan::{
        ...
        MarkPriceCaptureSpec, QuoteCaptureSpec,
    },
};
...
fn default_catalog_dir() -> Result<PathBuf> {
    ...
    Ok(PathBuf::from(format!(
        "/tmp/nautilus-binance-futures-derivatives-state-{ts}"
    )))
}
...
async fn main() -> Result<()> {
    ...
    let capture_actor = CatalogCaptureActor::new(CatalogCaptureActorConfig {
        actor_id: Some(ActorId::from("CATALOG_CAPTURE-BINANCE_DERIV_STATE")),
    ...
    let mut node = LiveNode::builder(trader_id, Environment::Live)?
        .with_name("BINANCE-FUTURES-DERIV-STATE-001")
    ...
    println!("Starting Binance Futures derivatives-state capture (Step 1)");
    println!("Catalog dir: {}", catalog_dir.display());
    println!("Instrument: {instrument_id}");
    println!("Families: instruments, quotes, mark_prices, index_prices, funding_rates");
    ...
    println!(
        "Verify with: /Users/yfclark/nautilus_trader/.venv/bin/python /Users/yfclark/nautilus_catalog_capture/tests/python_catalog_derivatives_probe.py {} {} 1",
        catalog_dir.display(),
        instrument_id
    );
