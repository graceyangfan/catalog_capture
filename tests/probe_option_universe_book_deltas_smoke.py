#!/usr/bin/env python3
"""Run 3-minute live option-universe captures with selective book_deltas validation."""
...
BOOK_DELTAS_SMOKE_VENUES = _cli_validate_mod.BOOK_DELTAS_SMOKE_VENUES
BOOK_DELTAS_SMOKE_PRESET = _cli_validate_mod.BOOK_DELTAS_SMOKE_PRESET
...
BOOK_DELTA_FAMILY_NAMES = ("order_book_deltas", "order_book_delta")


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run live option-universe book_deltas capture smokes (Step 6d, default 3 minutes)."
        ),
    )
    parser.add_argument(
        "--venue",
        choices=(*sorted(BOOK_DELTAS_SMOKE_VENUES), "all"),
        default="all",
        help="Venue profile to run. 'all' currently runs deribit-book-deltas only.",
    )
    parser.add_argument(
        "--seconds",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum order book delta rows required for at least one option.",
    )
    parser.add_argument(
        "--catalog-root",
        default="/tmp",
    ...
    if args.min_book_delta_rows <= 0:
        parser.error("--min-book-delta-rows must be positive")

    venues = sorted(BOOK_DELTAS_SMOKE_VENUES) if args.venue == "all" else [args.venue]
    failures = []
    for venue in venues:
        try:
            run_book_deltas_venue_smoke(venue, args)
...
def run_book_deltas_venue_smoke(venue: str, args: argparse.Namespace) -> None:
    ...
    temp_config = (
        Path(args.catalog_root)
        / f"capture.{venue}-btc-universe-book-deltas-smoke.{timestamp}.toml"
    )
    ...
    assert_book_delta_family_present(venue, summary)
    ...
    run_validation_suite(
        ...
        preset_override=BOOK_DELTAS_SMOKE_PRESET,
    )
    ...
    if not args.skip_readback_probe:
        probe_cmd = [
            ...
            str(args.min_book_delta_rows),
            "--book-deltas-only",
        ]
        for option_id in readback_option_ids:
            probe_cmd.extend(["--option-id", option_id])
    ...
    print(f"[{venue}] option-universe book_deltas smoke succeeded")


def assert_book_delta_family_present(
    venue: str,
    summary: dict[str, dict[str, int | None]],
) -> None:
    for family in BOOK_DELTA_FAMILY_NAMES:
        stats = summary.get(family)
        if not stats or int(stats.get("files", 0)) == 0:
            continue
        sample_rows = stats.get("sample_rows_first_5")
        print(
            f"[{venue}] book_deltas: family={family} files={stats['files']} "
            f"sample_rows_first_5={sample_rows}",
            flush=True,
        )
        if sample_rows is not None and int(sample_rows) == 0:
            raise RuntimeError(
                f"[{venue}] order book delta parquet family {family} had zero sample rows"
            )
        return

    raise RuntimeError(
        f"[{venue}] missing order_book_deltas parquet family; families={sorted(summary)}"
    )
